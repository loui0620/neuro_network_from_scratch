...
def train(model, X, y, num_passes=10000, reg_lambda = .1, learning_rate=0.1):
    ...
    #while i < 1500:
        ...
        if i % 1000 == 0:
            ...
            if (previous_loss-float(loss))/previous_loss < 0.001:
                done = True
                #print i
            previous_loss = loss
        i += 1
...
def main():
    ...
    X, y, num_labels = dataLoader(dataset_path) # import training data and labels
    ...
    model = build_model(X,20,3)
    model, losses, I = train(model,X, y, reg_lambda=reg_lambda, learning_rate=learning_rate)
    output = feed_forward(model, X)
    preds = np.argmax(output[3], axis=1)

    accuracy = np.sum(np.argmax(losses, axis=0) == np.argmax(preds, axis=0))
    #accuracy = accuracy * num_labels / 2
    accuracy /= len(X)
